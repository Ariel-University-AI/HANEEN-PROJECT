...
def _future_full_forecast(month: int, hour: int, dow: int) -> pd.DataFrame:
    ...
    for year in range(2023, 2036):
        ...
        zf["year"]               = float(year)
        zf["hour_sin"]           = np.sin(2 * np.pi * hour  / 24)
        zf["hour_cos"]           = np.cos(2 * np.pi * hour  / 24)
        zf["dow_sin"]            = np.sin(2 * np.pi * dow   / 7)
        zf["dow_cos"]            = np.cos(2 * np.pi * dow   / 7)
        zf["month_sin"]          = np.sin(2 * np.pi * month / 12)
        zf["month_cos"]          = np.cos(2 * np.pi * month / 12)
